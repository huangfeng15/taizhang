...
## 概述
定义系统中模态框（Modal Dialog）的管理规范，确保模态框的打开、关闭、切换等操作不会导致遮罩层叠加或页面无法操作的问题。
...
const CustomDialog = {
    ...
    // 显示警告模态框
    alert: function(options) { /* 返回 Promise */ },
    
    // 显示确认模态框
    confirm: function(options) { /* 返回 Promise */ },
    
    // 显示输入模态框
    prompt: function(options) { /* 返回 Promise */ },
...
### 必须通过的测试场景
1. 快速连续打开两个模态框，不出现遮罩层叠加
2. 删除失败后错误提示正常显示，关闭后页面可操作
3. 按 ESC 键可以关闭任何模态框
4. 点击遮罩层可以关闭模态框
5. 页面刷新后不会有残留的遮罩层
...
## 相关文件
- `project/static/js/custom-dialog.js` - 模态框组件实现
- `project/static/css/custom-dialog.css` - 模态框样式
