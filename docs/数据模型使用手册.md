...
1. 模型字段
   - 统一使用 `snake_case`，全部小写 + 下划线，不使用中文，不混用驼峰。
   - 以业务语义为主，优先以 Django 模型字段为“唯一真相”，GET 参数和表单 `name` 应与之保持一致。
   - 常见后缀约定：
     - `_code`：业务编号，如 `project_code`、`procurement_code`、`contract_code`、`payment_code`、`settlement_code`、`evaluation_code`。
     - `_name`：名称类字段，如 `project_name`、`contract_name`、`supplier_name`。
     - `_date`：日期，如 `signing_date`、`bid_opening_date`、`archive_date`、`completion_date`。
     - `_amount` / `_price` / `_ratio` / `_count`：金额、价格、比例、数量，如 `contract_amount`、`budget_amount`、`control_price`、`payment_ratio` 等。
     - 布尔值使用 `is_` / `has_` 前缀，如 `is_settled`、`has_complaint`（已有字段中存在少量历史例外）。
   - 跨模块共用字段优先采用统一命名，例如：
     - 项目：`project_code` / `project_name`
     - 采购：`procurement_code` / `procurement_officer`
     - 合同：`contract_code` / `contract_officer`
   - 避免含义不清的缩写（如 `cont_off`）；确需缩写时，应在本手册中统一说明后再使用。

2. 筛选参数 / 表单字段
   - 文本 / 枚举筛选：参数名 = 模型字段名，例如：
     - `procurement_officer`
     - `party_b_contact_person`
   - 数值区间：使用 `{field}_min` / `{field}_max`，例如：
     - `contract_amount_min` / `contract_amount_max`
     - `budget_amount_min` / `budget_amount_max`
   - 日期区间：使用 `{field}_start` / `{field}_end`，例如：
     - `signing_date_start` / `signing_date_end`
     - `result_publicity_release_date_start` / `result_publicity_release_date_end`
   - 多选下拉：前端表单仍使用字段名本身，后端使用 `request.GET.getlist('{field}')` 读取，例如：
     - `contract_type`、`file_positioning` 等枚举类字段。
   - 所有**新增筛选字段**必须同时满足：
     - 在 `project/filter_config.py` 中声明；
     - 在对应视图（如 `views_contracts.py` / `views_procurements.py`）中显式读取并应用到 queryset 过滤。
   - 可通过管理命令：
     - `python manage.py check_filter_mappings`
     自动检查 `filter_config` 中定义的筛选字段与视图中的 GET 参数是否一一对应。

### 0.2 文件与目录命名规范

1. Django 应用与模块
   - 应用目录使用单数领域名，如：`project`、`procurement`、`contract`、`payment`、`settlement`、`supplier_eval`。
   - Python 模块文件统一使用 `snake_case`，例如：
     - `views_contracts.py`、`views_procurements.py`
     - `filter_config.py`、`views_helpers.py`
   - 领域服务 / 工具模块推荐命名方式：
     - `services/export_service.py`、`services/monitoring_service.py` 等，以用途+领域命名。

2. 模板与静态资源
   - 模板文件使用 `snake_case` + 领域前缀，例如：
     - 列表页：`procurement_list.html`、`contract_list.html`
     - 组件：`components/filter_system.html`
   - 静态资源（JS/CSS）推荐 `kebab-case`，例如：
     - `contract-list.js`、`filter-system.css`

3. 文档与脚本
   - 文档（`docs/`）使用有语义的文件名，避免空格，可中英文混合，例如：
     - `数据模型使用手册.md`、`time-range-filter-design.md`、`指标完善.md`
   - 脚本（`scripts/`）统一使用 `snake_case`，以动词开头说明用途，例如：
     - `generate_ssl_cert.py`、`restart_server.bat`

### 0.3 现有字段（历史命名，不强制重命名）

为避免大规模重构对现网数据与代码的影响，下列字段视为**历史命名**，不强制按本规范重命名；但在新代码中引用时必须保持一致。新增字段一律遵循 0.1 / 0.2 中的命名规则。

1. Project（项目）

- `project_code`, `project_name`, `description`, `project_manager`, `status`, `remarks`
- 审计字段（由 `AuditBaseModel` 提供）：`created_at`, `updated_at`, `created_by`, `updated_by`

2. Procurement（采购）

- 主键与关联：
  - `procurement_code`, `project`
- 基本信息：
  - `project_name`, `procurement_unit`, `procurement_category`, `procurement_platform`
- 方式与审查：
  - `procurement_method`, `qualification_review_method`, `bid_evaluation_method`, `bid_awarding_method`
- 金额信息：
  - `budget_amount`, `control_price`, `winning_amount`
- 人员信息：
  - `procurement_officer`, `demand_department`, `demand_contact`
- 中标信息：
  - `winning_bidder`, `winning_contact`
- 时间信息：
  - `planned_completion_date`, `requirement_approval_date`, `announcement_release_date`,
    `registration_deadline`, `bid_opening_date`, `candidate_publicity_end_date`,
    `result_publicity_release_date`, `notice_issue_date`, `archive_date`
- 评审与担保：
  - `evaluation_committee`, `bid_guarantee`, `bid_guarantee_return_date`, `performance_guarantee`
- 其他信息：
  - `candidate_publicity_issue`, `non_bidding_explanation`
- 审计字段继承自 `BaseModel`

3. Contract（合同）

- 主键与关联：
  - `contract_code`, `project`, `procurement`, `parent_contract`
- 基本与分类信息：
  - `contract_name`, `file_positioning`, `contract_type`, `contract_source`, `contract_sequence`
- 合同方信息：
  - `party_a`, `party_b`
- 甲方联系信息：
...
- 金额与时间：
  - `contract_amount`, `signing_date`, `duration`, `performance_guarantee_return_date`, `archive_date`
- 其他信息：
  - `contract_officer`, `payment_method`
- 审计字段继承自 `BaseModel`

4. Payment（付款）

- 主键与关联：
  - `payment_code`, `contract`
- 付款信息：
  - `payment_amount`, `payment_date`
- 结算信息：
  - `settlement_amount`, `is_settled`, `settlement_archive_date`
- 审计字段继承自 `BaseModel`

5. Settlement（结算）

- 主键与关联：
  - `settlement_code`, `main_contract`
- 结算信息：
  - `final_amount`, `completion_date`, `remarks`
- 审计字段继承自 `BaseModel`

6. SupplierEvaluation（供应商履约评价）

- 主键与关联：
  - `evaluation_code`, `contract`, `supplier_name`
- 兼容的评价字段：
  - `evaluation_period`, `evaluator`, `score`, `evaluation_type`
- 扩展评分字段：
  - `comprehensive_score`, `last_evaluation_score`,
    `annual_scores`, `irregular_scores`
- 其他：
  - `remarks`
- 审计字段继承自 `BaseModel`

> 说明：上述字段在系统中已广泛使用，除非进行专门的数据迁移与重构，否则保持现有命名不变。新增字段、文件和目录应优先遵循 0.1 / 0.2 的规范，以逐步提升整体一致性与可维护性。

---

### 0.4 命名规范 Code Review 检查清单

在 Code Review 时，可按下列要点快速自查命名是否符合规范：

1. **模型字段**
   - [ ] 新增字段是否使用 `snake_case`，且无中文/驼峰/缩写含义不明的命名。
   - [ ] 后缀是否符合语义（`_code` / `_name` / `_date` / `_amount` / `_price` / `_ratio` / `_count` / `is_*` / `has_*`）。
   - [ ] 是否复用已有领域字段命名（如 `project_code`、`procurement_officer`、`contract_officer`），避免创造近义新名。

2. **筛选参数 & 表单字段**
   - [ ] `filter_config` 中的 `name` 是否与模型字段名一致（文本/枚举类）。
   - [ ] 数值和日期范围是否使用 `{field}_min` / `{field}_max`、`{field}_start` / `{field}_end` 格式。
   - [ ] 新增筛选字段是否已经在对应视图中通过 `request.GET.get(...)` / `getlist(...)` 读取并参与 queryset 过滤。
   - [ ] 模板中的表单 `name` 是否与视图读取的 GET 参数完全一致。

3. **文件 / 目录命名**
   - [ ] 新增 Python 模块是否使用 `snake_case`，且位于合理的应用目录（如 `project/`、`procurement/` 等）。
   - [ ] 模板文件是否采用 `snake_case` + 领域前缀（如 `contract_list.html`、`components/filter_system.html`）。
   - [ ] 新增文档是否放在 `docs/` 下，并使用有语义、易检索的文件名。

4. **工具与自动检查**
   - [ ] 新增或修改高级筛选后，是否已运行 `python manage.py check_filter_mappings`，确保 `filter_config` 与视图 GET 参数一一对应。
   - [ ] 若对历史字段命名有变更，是否评估了数据迁移影响，并在本手册中同步更新“0.3 现有字段”。

> 建议：将本检查清单作为 PR 模板的一个小节，在提交前由开发者自勾选，可显著降低命名相关的隐性缺陷。
...
| `party_b_manager` | CharField(200) | Optional | '' | 乙方负责人及联系方式 |
...
### 2.14 工作周期监控服务（CycleStatisticsService）

#### 2.14.1 服务定位

`CycleStatisticsService`是专门用于监控采购和合同工作周期的服务类。计算采购周期（需求书审批→结果公示）和合同周期（结果公示→合同签订）的统计数据，支持按时完成率分析和周期预警。

#### 2.14.2 核心方法

| 方法名 | 参数 | 返回类型 | 功能说明 | 业务逻辑 |
|--------|------|----------|----------|----------|
| `get_projects_cycle_overview()` | year_filter, project_filter, procurement_methods, group_by | dict | 获取项目维度的周期概览 | 统计项目下采购和合同的周期及按时率 |
| `get_persons_cycle_overview()` | year_filter, project_filter, procurement_methods, group_by | dict | 获取人员维度的周期概览 | 统计经办人的工作周期表现 |
| `get_project_cycle_detail()` | project_code, year_filter, procurement_methods, group_by | dict | 获取项目周期详细数据 | 项目下所有采购/合同的周期明细 |
| `get_person_cycle_detail()` | person_name, year_filter, project_filter, procurement_methods, group_by | dict | 获取人员周期详细数据 | 经办人负责的所有业务周期明细 |
| `get_procurement_deadline()` | procurement_method | int | 根据采购方式获取规定周期天数 | 从配置中获取各采购方式的规定周期 |
...
```python
# 创建周期统计服务实例
service = CycleStatisticsService()

# 获取项目周期概览
overview = service.get_projects_cycle_overview(
    year_filter='2025',
    project_filter='PRJ2025001',
    procurement_methods=['公开招标', '竞争性谈判'],
    group_by='procurement_method'
)

# 获取人员周期概览
person_overview = service.get_persons_cycle_overview(
    year_filter='2025'
)

# 获取项目周期详情
detail = service.get_project_cycle_detail(
    project_code='PRJ2025001',
    year_filter='2025'
)

# 获取采购方式规定周期
deadline = service.get_procurement_deadline('公开招标')  # 返回45
```
...
# 合同周期计算
contract_cycle = (
    signing_date - result_publicity_release_date
).days

# 按时率计算
if actual_cycle <= deadline_days:
    on_time = True
else:
    on_time = False
```
...
### 2.15 工作周期监控视图（cycle_monitor）

#### 2.15.1 视图定位

`cycle_monitor`是工作周期监控页面的视图函数，支持项目视图和人员视图两种模式，提供周期数据的概览和详情查看功能。
...
def cycle_monitor(request):
    ...
    - target_code: 目标编码（项目编码或人员姓名）
    - procurement_method: 采购方式筛选（支持多选）
    - group_by: 分组方式（支持按采购方式分组）

    返回:
    - 概览数据：projects/persons维度统计
    - 详情数据：选中项目/人员的周期明细
    - 趋势数据：按月周期的变化趋势
    """
```
...
- **项目视图模式**：按项目统计采购周期和合同周期，支持项目间的周期表现对比
- **人员视图模式**：按经办人统计周期表现，识别高效和低效人员
- **采购方式筛选**：支持按具体采购方式筛选统计范围
- **分组统计**：可按采购方式分组查看周期数据
- **详情查看**：点击项目或人员可查看其周期明细和趋势图
...
context = {
    'page_title': '工作周期监控',
    'view_mode': view_mode,                    # 'project' | 'person'
    'target_code': target_code,               # 选中的项目编码或人员姓名
    'overview_data': overview_data,           # 概览统计数据
    'detail_data': detail_data,               # 详情数据（可选）
    'trend_data_json': trend_data_json,       # 趋势图JSON数据
    'cycle_rules': CYCLE_RULES,               # 周期规则配置
    'selected_procurement_methods': procurement_methods,  # 选中的采购方式
    'group_by': group_by,                     # 分组方式
}
```
...
```python
# 项目视图
/conitoring/cycle/?view_mode=project&target_code=PRJ2025001

# 人员视图
/conitoring/cycle/?view_mode=person&target_code=张三

# 按采购方式筛选
/conitoring/cycle/?procurement_method=公开招标&procurement_method=竞争性谈判

# 分组统计
/conitoring/cycle/?group_by=procurement_method
```

---

### 2.16 供应商评价列表视图（supplier_evaluation_list 重构）

#### 2.16.1 视图重构说明

`supplier_evaluation_list`视图已完成重构，支持全局年度筛选，提供更灵活的年度评价数据查看方式。

#### 2.16.2 新增功能特性

**全局年度筛选支持**
```python
# 全部年度：展示每个供应商的最新一次评价记录
year_filter = None
latest_evaluations = SupplierAnalysisService.get_latest_evaluations_by_year(None)

# 特定年度：仅展示该年度的评价记录
year_filter = 2025
year_evaluations = SupplierEvaluation.objects.filter(created_at__year=2025)
```

**智能数据获取逻辑**
- 当选择"全部年度"时，自动获取每个供应商的最新评价记录
- 当选择具体年份时，展示该年度的所有评价记录
- 支持项目联动筛选，结合全局项目筛选器
- 支持供应商名称模糊搜索
...
```python
# 四个评分等级
excellent: comprehensive_score >= 90    # 优秀
good: 80 <= comprehensive_score < 90    # 良好
qualified: 70 <= comprehensive_score < 80  # 合格
unqualified: comprehensive_score < 70   # 不合格
```
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `year_filter` | int/None | 年度筛选，None表示全部年度 |
| `year_value` | str | 年份显示值（'all'或具体年份） |
| `project_list` | list | 项目筛选列表 |
| `q` | str | 供应商名称搜索关键词 |
| `score_level` | str | 评分等级筛选 |
...
```python
# 查看全部年度最新评价
/supplier/evaluation/?year=all

# 查看2025年度评价
/supplier/evaluation/?year=2025&project=PRJ2025001

# 筛选优秀评价
/supplier/evaluation/?year=2025&score_level=excellent

# 搜索供应商
/supplier/evaluation/?q=某某公司&year=2025
```

---

### 2.17 供应商评价详情视图（supplier_evaluation_detail 新增）

#### 2.17.1 视图定位

`supplier_evaluation_detail`是新增的供应商评价详情页面视图，展示指定供应商在所有项目中的完整履约评价历史记录。
...
summary = {
    'total_contracts': int,        # 合同总数
    'total_amount': Decimal,       # 合同总金额（万元）
    'avg_score': float,            # 平均评分
    'evaluation_count': int,       # 评价次数
    'excellent_count': int,        # 优秀次数
    'good_count': int,             # 良好次数
    'qualified_count': int,        # 合格次数
    'unqualified_count': int,      # 不合格次数
}
```
...
- **分页展示**：历史评价记录分页显示，每页20条
- **详情查看**：点击评价记录可查看详细评价信息
- **趋势图表**：使用Chart.js展示年度评分趋势
- **汇总统计**：页面顶部展示供应商整体表现
- **金额换算**：自动将合同总金额转换为万元单位

#### 2.17.5 使用示例

```python
# 查看指定供应商的评价详情
/supplier/evaluation/detail/某某咨询公司

# 分页查看
/supplier/evaluation/detail/某某咨询公司/?page=2&page_size=20
```
...
context = {
    ...
    'evaluations': page_obj,                 # 分页评价记录
    'summary': summary,                     # 供应商汇总信息
    'stats': evaluation_stats,              # 评价统计数据
    'trend_chart_json': trend_chart_json,   # 趋势图JSON
    'total_evaluations': len(all_evaluations),  # 总记录数
}
```
