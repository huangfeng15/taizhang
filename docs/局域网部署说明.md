...
## 概述
本文档说明如何将Django项目部署到本地局域网，使局域网内其他设备可以访问。系统使用HTTPS协议提供安全访问。
...
1. 检查并生成SSL证书（如不存在）
2. 执行数据库迁移
3. 启动HTTPS服务器（监听 0.0.0.0:3500）

启动后会显示：
```
可用的访问地址：
  - 本机访问: https://127.0.0.1:3500/
  - 局域网访问: https://10.168.3.240:3500/
  - 其他电脑访问: https://10.168.3.240:3500/

Note: Browser may show security warning due to self-signed certificate.
Please click "Continue" or "Trust this site"
```
...
**注意**: 手动启动仅支持HTTP协议，推荐使用 `start_server.bat` 以获得HTTPS支持。
...
**局域网访问（其他设备）：**
- https://10.168.3.240:3500/ （替换为实际IP）

**系统功能入口：**
- **管理后台**: https://10.168.3.240:3500/admin/
- **项目台账**: https://10.168.3.240:3500/
- **数据监控**: https://10.168.3.240:3500/monitoring/
  - 统计分析: https://10.168.3.240:3500/monitoring/statistics/
  - 完整性监控: https://10.168.3.240:3500/monitoring/completeness/
  - 供应商排名: https://10.168.3.240:3500/monitoring/ranking/
  - 更新监控: https://10.168.3.240:3500/monitoring/update/
- **报表导出**: https://10.168.3.240:3500/reports/

**⚠️ 安全警告提示**
首次访问时，浏览器会显示"您的连接不是私密连接"或类似警告，这是因为使用了自签名SSL证书。请点击"高级"→"继续访问"或"信任此站点"。

### 3. 移动设备访问

手机、平板等移动设备可通过浏览器访问：
- 确保设备连接到同一局域网（同一WiFi）
- 打开浏览器输入：https://10.168.3.240:3500
- 首次访问时接受安全证书警告
- 系统采用响应式设计，自动适配移动端
...
**症状：**
- 本机可以访问 https://127.0.0.1:3500
- 其他设备无法访问 https://服务器IP:3500

**解决方案：**

1. **检查服务器启动方式**
   ```bash
   # 推荐：使用 start_server.bat 启动（自动使用 0.0.0.0:3500）
   start_server.bat  # ✅ 正确
   
   # 或手动启动（确认使用 0.0.0.0 而不是 127.0.0.1）
   python manage.py runserver_plus --cert-file ssl_certs\server.crt --key-file ssl_certs\server.key 0.0.0.0:3500  # ✅ 正确
   python manage.py runserver 127.0.0.1:3500  # ❌ 错误（仅本机，且仅HTTP）
   ```
