...
1. 确保防火墙允许8000端口
2. 启动服务会自动监听 `0.0.0.0:8000`

```bash
# Windows防火墙规则（管理员权限）
netsh advfirewall firewall add rule name="Django HTTPS" dir=in action=allow protocol=TCP localport=8000
```

### 客户端

1. 获取服务器IP地址（如 `192.168.1.100`）
2. 访问 `https://192.168.1.100:8000`
3. 信任证书（同上述浏览器步骤）
