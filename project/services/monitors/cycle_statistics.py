...
class CycleStatisticsService:
    ...
    def _group_by_month(self, queryset):
        ...
        for month in range(1, 13):
            ...
            if count > 0:
                avg_cycle_timedelta = month_data.aggregate(avg=Avg('work_cycle'))['avg']
    ...
    def _group_by_half_year(self, queryset):
        ...
        for year in years:
            ...
            if h1_count > 0:
                h1_avg_timedelta = h1_data.aggregate(avg=Avg('work_cycle'))['avg']
            ...
            if h2_count > 0:
                h2_avg_timedelta = h2_data.aggregate(avg=Avg('work_cycle'))['avg']
    ...
    def get_person_list(self, year_filter=None, global_project=None, procurement_method=None):
        ...
        if year_filter and year_filter != 'all':
            procurement_officers = procurement_officers.filter(
                requirement_approval_date__year=int(year_filter)
            )
