...
```python
# monitoring/services/update_service.py
...
class UpdateMonitorService:
    ...
    def get_project_update_status(self, project_id=None):
        """获取项目数据更新状态
        ...
        Returns:
            ...
            - project_name: 项目名称
            - project_status: 项目状态
            - procurement_last_update: 采购最后更新
            - contract_last_update: 合同最后更新
            - payment_last_update: 付款最后更新
            - settlement_last_update: 结算最后更新
            - alert_level: 预警级别 (normal/warning/critical)
        """
        projects = Project.objects.all()
        if project_id:
            projects = projects.filter(project_code=project_id)
        ...
        for project in projects:
            ...
            # 采购模块最后更新
            procurement_update = project.procurements.aggregate(
                last_update=Max('updated_at')
            )['last_update']
            status['procurement_last_update'] = procurement_update
            status['procurement_days'] = self._calculate_days_ago(procurement_update)
            
            # 合同模块最后更新
            contract_update = project.contracts.aggregate(
                last_update=Max('updated_at')
            )['last_update']
            status['contract_last_update'] = contract_update
            status['contract_days'] = self._calculate_days_ago(contract_update)
            
            # 付款模块最后更新
            payment_update = None
            for contract in project.contracts.all():
                contract_payment_update = contract.payments.aggregate(
                    last_update=Max('updated_at')
                )['last_update']
                if payment_update is None or (contract_payment_update and contract_payment_update > payment_update):
                    payment_update = contract_payment_update
            status['payment_last_update'] = payment_update
            status['payment_days'] = self._calculate_days_ago(payment_update)
            ...
            for contract in project.contracts.filter(contract_type='主合同'):
                try:
                    ...
                    if settlement:
                        if settlement_update is None or settlement.updated_at > settlement_update:
                            settlement_update = settlement.updated_at
                except:
                    pass
            status['settlement_last_update'] = settlement_update
            status['settlement_days'] = self._calculate_days_ago(settlement_update)
            
            # 计算预警级别
            status['alert_level'] = self._calculate_alert_level(status)
            
            result.append(status)
        
        return result
    
    def _calculate_days_ago(self, dt):
        """计算距今天数"""
        if dt is None:
            return None
        return (timezone.now() - dt).days
    
    def _calculate_alert_level(self, status):
        """计算预警级别"""
        # 超过40天为critical，30-40天为warning
        days_list = [
            status.get('procurement_days'),
            status.get('contract_days'),
            status.get('payment_days'),
            status.get('settlement_days')
        ]
